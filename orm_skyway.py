...
def write_gilhari_artifacts(cfg: dict, config_path: Path, class_names: list):
    ...
    _dockerfile_lines = [
        ...
        f"ARG BASE_PLATFORM=linux/amd64",
    ...
    (root / "build.cmd").write_text(
        ...
        f"docker build --platform linux/amd64 -t {image_name}:{image_tag} .\r\n"
        f"docker images\r\n",
        encoding="utf-8"
    )
    ...
    (root / "run_docker_app.cmd").write_text(
        ...
        f"docker run --platform linux/amd64 -d --name {image_name}{_mount_flag_cmd} -p {host_port}:{service_port} {image_name}:{image_tag}\r\n"
        f"\r\n"
    ...
    write_sh(root / "run_docker_app.sh",
        ...
        f"docker run --platform linux/amd64{_add_host_flag} -d --name {image_name}{_mount_flag_sh} -p {host_port}:{service_port} {image_name}:{image_tag}\n"
        f"\n"
        f"echo \"Waiting for Gilhari microservice to start...\"\n"
        f"echo \"(This may take up to 3 minutes for cloud or remote databases)\"\n"
    ...
    if yn_confirm("Run 'docker build' now?", default=True):
        cmd = ["docker", "build", "--platform", "linux/amd64", "-t", f"{image_name}:{image_tag}", "."]
